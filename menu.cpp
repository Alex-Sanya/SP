	...
	s_enemy** save_first_enemy)
{
	...
	game_process(map, player, screen_pos, q_stone, first_enemy, save_map, save_player, save_q_stone, save_first_enemy, *all_colors);
...
int game_process(s_map* map, s_player* player, COORD* screen_pos, s_q_stone* q_stone, s_enemy** first_enemy,
	s_map* save_map, s_player* save_player, s_q_stone* save_q_stone, s_enemy** save_first_enemy, s_all_colors all_colors)
{
	...
	while (1)
	{
		...
		if (clock() - now_time > 250) // обновление раз в четверть секунды
		{
			...
			print_map(map, screen_pos, player, first_enemy, all_colors); // нарисовать карту
